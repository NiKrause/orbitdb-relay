...
- **Sync bounds:** `RELAY_ORBITDB_SYNC_TIMEOUT_MS`, `RELAY_ORBITDB_SYNC_COOLDOWN_MS`, `RELAY_ORBITDB_SYNC_MAX_COOLDOWN_MS`, `RELAY_PINNING_SYNC_TIMEOUT_MS` — keep one unsyncable database from stalling pubsub-driven discovery for the whole node (see `src/config/orbitdb-sync-env.ts`).
